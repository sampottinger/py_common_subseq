...
class TestCalcACS(unittest.TestCase):
    """Test suite for the calc_acs_dp mini-module."""

    def test_count_common_subsequences(self):
        """Test counting the number of subsequences without listing them."""
        no_shared = calc_acs_dp.count_common_subsequences(NONE_SHARED_1,
            NONE_SHARED_2)
        self.assertEqual(no_shared, 1)

        partially_shared = calc_acs_dp.count_common_subsequences(
            PARTIALLY_SHARED_1, PARTIALLY_SHARED_2)
        self.assertEqual(partially_shared, 12)

        all_shared = calc_acs_dp.count_common_subsequences(ALL_SHARED_1,
            ALL_SHARED_2)
    ...
    def test_find_common_subsequences(self):
        ...
        no_shared = calc_acs_dp.find_common_subsequences(NONE_SHARED_1,
            NONE_SHARED_2)
        self.assertEqual(no_shared, set(['']))

        partially_shared = calc_acs_dp.find_common_subsequences(
            PARTIALLY_SHARED_1, PARTIALLY_SHARED_2)
        components = ['', 'qer', 'wr', 'qwr', 'er', 'qr', 'e', 'qw', 'q', 'r',
            'qe', 'w']
        self.assertEqual(partially_shared, set(components))

        all_shared = calc_acs_dp.find_common_subsequences(ALL_SHARED_1,
            ALL_SHARED_2)
